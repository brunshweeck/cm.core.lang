...
#ifndef CORE_LANG_LONGARRAY_H
#define CORE_LANG_LONGARRAY_H
#include "Integer.h"
#include "Long.h"
...
namespace core
{
    /// The class {@code LongArray} wraps the array of a primitive type {@code long} in the object.
    /// An object of type {@code LongArray} contains two fields whose first is a pointer of {@code long}, and the second is the number of places allocated in this pointer.
    ...
    class LongArray $final : public lang::spi::AbstractPrimitiveArray<Long, LongArray>
    {
        class ReferenceStep;
        class ConstantStep;

    public:
        /// Construct new empty {@code LongArray}.
        LongArray() = default;

        /// Construct new {@code LongArray} and initialize content with given values.
        /// @param values the list of value to be used to initialize elements of this array.
        LongArray(VarArgs values);

        /// Construct new {@code LongArray} able to contain the given number of values.
        ...
        CORE_EXPLICIT LongArray(gint length, glong initializer = 0L);

        /// Construct new  {@code LongArray} containing the elements of specified array; in other they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into a resulting array.
        LongArray(const LongArray& other);

        /// Set the content of this {@code LongArray} with its specified array; During this operation, the elements will be copied to this array in order they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into this array as new content.
        /// @return this array
        LongArray& operator=(const LongArray& other);

        /// Construct new  {@code LongArray} containing the elements of specified array; in other they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into a resulting array.
        LongArray(LongArray&& other) CORE_NOTHROW;

        /// Set the content of this {@code LongArray} with its specified array; During this operation, the elements will be copied to this array in order they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into this array as new content.
        /// @return this array
        LongArray& operator=(LongArray&& other) CORE_NOTHROW;
        ...
        /// @return the shadow copy of this array.
        LongArray& clone() const override;
        ...
        template <class... Args>
        static LongArray of(const Args&... args);
        ...
        static LongArray ofRange(glong start, glong end, glong offsetByValue = 1L);
        ...
        /// @note The value {@code 0L} will replace each undefined element of a given array.
        /// @param array an array from which elements are drawn.
        /// @return an array containing the elements of the given array.
        static LongArray copyOf(const Array<Long>& array);
        ...
        static LongArray copyOf(const Array& array);

        CORE_INTRINSIC_INDEXED_ITEM_ACCESS_DECL(glong, gint);

        CORE_INTRINSIC_FOR_EACH_DECL(ConstantStep, ReferenceStep);
    };

    class LongArray::ReferenceStep $final : public AbstractStep
    {
        LongArray& array;
        gint index;

    public:
        CORE_EXPLICIT ReferenceStep(LongArray& array, gboolean gotoEnd);
    ...
    class LongArray::ConstantStep $final : public AbstractConstantStep
    {
        const LongArray& array;
        gint index;

    public:
        CORE_EXPLICIT ConstantStep(const LongArray& array, gboolean gotoEnd);

        ~ConstantStep() override = default;

        const glong& get() override;
