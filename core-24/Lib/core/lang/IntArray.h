...
#ifndef CORE_LANG_INTARRAY_H
#define CORE_LANG_INTARRAY_H
#include "Integer.h"
#include "Integer.h"
...
namespace core
{
    /// The class {@code IntArray} wraps the array of a primitive type {@code int} in the object.
    /// An object of type {@code IntArray} contains two fields whose first is a pointer of {@code int}, and the second is the number of places allocated in this pointer.
    ...
    class IntArray $final : public lang::spi::AbstractPrimitiveArray<Integer, IntArray>
    {
        class ReferenceStep;
        class ConstantStep;

    public:
        /// Construct new empty {@code IntArray}.
        IntArray() = default;

        /// Construct new {@code IntArray} and initialize content with given values.
        /// @param values the list of value to be used to initialize elements of this array.
        IntArray(VarArgs values);

        /// Construct new {@code IntArray} able to contain the given number of values.
        ...
        CORE_EXPLICIT IntArray(gint length, gint initializer = 0);

        /// Construct new  {@code IntArray} containing the elements of specified array; in other they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into a resulting array.
        IntArray(const IntArray& other);

        /// Set the content of this {@code IntArray} with its specified array; During this operation, the elements will be copied to this array in order they are returned by the array's iterator.
        ...
        /// Construct new  {@code IntArray} containing the elements of specified array; in other they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into a resulting array.
        IntArray(IntArray&& other) CORE_NOTHROW;

        /// Set the content of this {@code IntArray} with its specified array; During this operation, the elements will be copied to this array in order they are returned by the array's iterator.
        /// @param other the array whose elements are to be placed into this array as new content.
        /// @return this array
        IntArray& operator=(IntArray&& other) CORE_NOTHROW;
        ...
        template <class... Args>
        static IntArray of(const Args&... args);
        ...
        static IntArray ofRange(gint start, gint end, gint offsetByValue = 1);
        ...
        static IntArray copyOf(const Array<Integer>& array);
        ...
        static IntArray copyOf(const Array& array);

        CORE_INTRINSIC_INDEXED_ITEM_ACCESS_DECL(gint, gint);

        CORE_INTRINSIC_FOR_EACH_DECL(ConstantStep, ReferenceStep);
    };

    class IntArray::ReferenceStep $final : public AbstractStep
    {
        IntArray& array;
        gint index;

    public:
        CORE_EXPLICIT ReferenceStep(IntArray& array, gboolean gotoEnd);
    ...
    class IntArray::ConstantStep $final : public AbstractConstantStep
    {
        const IntArray& array;
        gint index;

    public:
        CORE_EXPLICIT ConstantStep(const IntArray& array, gboolean gotoEnd);

        ~ConstantStep() override = default;

        const gint& get() override;
    ...
    IntArray IntArray::copyOf(const Array& array) {
        ...
        for (gint i = 0; i < length; i += 1) copy[i] = (gint)array[i];
        return copy;
    }
} // core

#endif // CORE_LANG_INTARRAY_H
