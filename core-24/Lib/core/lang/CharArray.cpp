...
namespace core
{
    CharArray::CharArray(VarArgs values) {
    ...
    CharArray::CharArray(gint length, gchar initializer) {
        if (length < 0)
    ...
    CharArray::CharArray(CharArray&& other) CORE_NOTHROW {
        gint length = other.length();
        AbstractPrimitiveArray::length = length;
    ...
    CharArray& CharArray::operator=(CharArray&& other) CORE_NOTHROW {
        if (this != &other) {
    ...
    gint CharArray::length() const { return AbstractPrimitiveArray::length; }

    gboolean CharArray::isEmpty() const { return length() == 0; }
    ...
    gchar CharArray::set(gint index, gchar newValue) {
        if (index >= length() || index < 0)
            ArrayIndexOutOfBoundsException(index).throws($ftrace());
        gchar oldValue = AbstractPrimitiveArray::values[index];
        AbstractPrimitiveArray::values[index] = newValue;
        return oldValue;
    }

    gboolean CharArray::equals(const Object& other) const {
        if (this == &other)
            return true;
        const CharArray& array = $cast(const CharArray&, other);
        gint numberOfElement = length();
        if (numberOfElement != array.length())
            return false;
        for (gint i = 0; i < numberOfElement; i += 1)
            if (Character::compare(array.values[i], AbstractPrimitiveArray::values[i]) != 0)
                return false;
        return true;
    }
    ...
    gint CharArray::hash() const {
        ...
        for (gint i = 0; i < length; i += 1)
            result = result * 31 + Character::hash(AbstractPrimitiveArray::values[i]);
        return result;
    }
    ...
    String CharArray::toString() const {
        ...
        for (gint i = 0; i < length; i += 1) {
            result += Character::toString(AbstractPrimitiveArray::values[i]);
            if (i != length - 1)
                result += ", "_Sl;
        }
        result += $toString(]);
    ...
    CharArray CharArray::ofRange(gchar start, gchar end, gint offsetByValue) {
        ...
        if (numberOfElements > (gchar)Integer::MAX_VALUE)
            OutOfMemoryError($toString(The array length required is too large)).throws($ftrace());
        gint length = (gint)numberOfElements;
        CharArray array = CharArray(length, start);
        for (gint i = 0; i < length; i += 1) array.values[i] += (gchar) (offsetByValue * i);
        return array;
    }

    CharArray CharArray::copyOf(const Array<Character>& array) {
        ...
        for (gint i = 0; i < length; i += 1) {
            ...
            copy.values[i] = item == null ? 0 : $cast(const Character&, item).charValue();
        }
        return copy;
    }

    CharArray::ReferenceStep::ReferenceStep(CharArray& array, gboolean gotoEnd)
        : array(array), index(gotoEnd ? Integer::MAX_VALUE : 0) {
    }

    gchar& CharArray::ReferenceStep::get() {
        if (index > array.length())
            IllegalStateException($toString()).throws($ftrace());
        return array[index];
    }

    CharArray::ReferenceStep& CharArray::ReferenceStep::next() {
        index = (index < Integer::MAX_VALUE && index >= array.length()) ? Integer::MAX_VALUE : index + 1;
        return *this;
    }

    gboolean CharArray::ReferenceStep::isDone() const { return index >= array.length(); }

    gint CharArray::ReferenceStep::position() const { return index; }

    const Object& CharArray::ReferenceStep::target() const { return array; }

    CharArray::ConstantStep::ConstantStep(const CharArray& array, gboolean gotoEnd)
        : array(array), index(gotoEnd ? Integer::MAX_VALUE : 0) {
    }

    const gchar& CharArray::ConstantStep::get() {
        if (index > array.length())
            IllegalStateException($toString()).throws($ftrace());
        return array[index];
    }

    CharArray::ConstantStep& CharArray::ConstantStep::next() {
        index = (index < Integer::MAX_VALUE && index >= array.length()) ? Integer::MAX_VALUE : index + 1;
        return *this;
    }

    gboolean CharArray::ConstantStep::isDone() const { return index >= array.length(); }

    gint CharArray::ConstantStep::position() const { return index; }

    const Object& CharArray::ConstantStep::target() const { return array; }

    CORE_INTRINSIC_INDEXED_ITEM_ACCESS_IMPL(gchar, gint, CharArray, get,)

    CORE_INTRINSIC_FOR_EACH_IMPL(
        CharArray::ConstantStep,
        CharArray::ReferenceStep,
        CharArray,
        (*this, false),
        (*this, true),
    )
